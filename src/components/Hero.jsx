import React from 'react';
import { styles } from '../styles';
import ComputersCanvas from './canvas/ComputersCanvas';
import { motion } from 'framer-motion'; // Ensure motion is imported from framer-motion

function Hero() {
  return (
    <section className='relative w-full h-screen mx-auto'>
      <div className={`${styles.paddingX} absolute inset-0 top-[120px] max-w-7xl mx-auto flex flex-row items-start gap-5`}>

        <div className='flex flex-col justify-center items-center mt-5'>
          {/* Ensure proper closing of div tags and use of style or class */}
          <div className='w-5 h-5 rounded-full' style={{ backgroundColor: '#91eff' }} />

          {/* Ensure proper closing of div tags */}
          <div className='w-1 sm:h-80 h-40 violet-gradient'></div>
        </div>

        <div>
          {/* Correct the Span tag to span and adjust the className */}
          <h1 className={`${styles.heroHeadText}`}>
            Hi, I'm <span className='text-[#915eff]'>Amir</span>
          </h1>
          <p className={`${styles.heroSubText} mt-2 text-[#0980d3]-100`}>
            I develop things and automate it <br className='sm:block hidden'/> using AWS
          </p>
        </div>
      </div>

      <ComputersCanvas/>

      <div className='absolute xs:bottom-1 bottom-32 w-full flex justify-center items-center'>

      <a href="#about">
        <div className='w-[35px] h-[64px] rounded-3xl border-4 border-secondary flex justify-center items-start p-2'>
          <motion.div animate={{
            y: [0, 24, 0]
          }}
          transition={{
            duration: 1.5,
            repeat: Infinity,
            repeatType: 'loop'
          }}
          className="w-3 h-3 rounded-full bg-secondary mb-1"
          />
        </div> 
      </a>
      </div>
    </section>
  );
}

export default Hero;
